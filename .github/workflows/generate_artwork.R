...
artwork <- switch(type,
  ...
  "12" = canvas_mandelbrot(colors = colorPalette("random-palette"), set = sample(c("mandelbrot", "multibrot", "julia"), size = 1), zoom = sample(seq(1, 2, by = 0.1), 1)),
  ...
  "32" = canvas_tiles(colors = colorPalette("random-palette"), iterations = sample(1000:10000, size = 1), duplicates = sample(3:5, size = 1), resolution = 100)
)
