...
artwork <- switch(type,
  ...
  "30" = canvas_flame(colors = colorPalette("random-palette"), background = sample(c("#fafafa", "firebrick", "#f9f0e0", "black", "lavenderblush2", "#215682"), size = 1), variations = sample(0:48, size = 3, replace = FALSE), iterations = 1e8, weighted = TRUE, post = sample(0:1, size = 1))
)
