...
artwork <- switch(type,
  ...
  "31" = canvas_smoke(colors = if (runif(1) <  2 / 3) colorPalette(sample(c("divergent", "random", "complement"), size = 1), n = 1024) else "all", compute = sample(c("minimum", "average"), size = 1), distance = sample(c("euclidean", "manhattan"), size = 1), resolution = 500)
)
